...
| PS-03 | Scope then remeasure | **Scope formula (lock):** union of (a) lane codes / registry keys / explicit paths named in the triggering utterance, (b) else active lanes in `wavves/INDEX.md` for this repo only. **Forbidden:** registry-wide crawl of every lane without named scope; transcript search as inventory source. **Remeasure inside scope only:** `wavves/registry.yml` status + `active_dispatch`; lane `dispatch*.md` / waveset open rows; named `*_OPERATOR_GATE.md` / lock ABSENT honesty; open FRs/handoffs **named in the trigger** (candidates only until `source_path` remeasures). Operator paste list = candidates; unverified names → `operator_gate` (“unverified standing claim”), never `dispatch`/`commit`. |
...
(Bare-shrug non-widen, standing path `wavves/standing/`, scope-then-remeasure,
