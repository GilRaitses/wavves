...
- **Product surface:** charter leave-acts / checkpoint; proceed standing remeasure;
...
2. **PAS** — proceed-all-standing moves from a standing file without remeasure
...
| KV-02 | proceed / PAS | Standing file is the program cache. Remeasure = invalidate+rewrite before move. Stale file without remeasure → `PROC-PROCEED-STALE-QUEUE`. Document as cache invalidation, not chat search. |
...
| `PROC-KV-STANDING-STALE` | all-standing move without remeasure against live registry/dispatch (bind PAS stale) |
...
2. Standing remeasure rule named as invalidation; fixture covers stale FAIL.
...
land status remeasured). No BUILD from this folder alone.
