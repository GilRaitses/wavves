...
**Evidence:** outbound copy lane P2-TUNNEL live run under originating product repo outbound copy lane home  
...
Not BLOCK: the live tunnel pattern worked once under operator clearance and the problem statement is real. Not GO: acceptance is a sketch without a harness, the closed fail vocab undershoots documented outbound copy REVISE bans, rewrite is self-graded under loop cap 1 with no post-cap path, and defaults assume the outbound email anatomy happy path.
...
- `<repo-root>/evals/` has `run_fixtures.py` and other fixtures; **no** paragraph-tunnel / PN-* fixture exists (glob over `evals/` + `*paragraph*tunnel*` → FR only).
- Live "proof" is agent JSON captures (outbound copy p2-adversarial and p2-rewrite captures), not a harness an independent agent can re-run.
...
**Evidence:**
- Adversarial capture has exactly three hand-shaped cases: high already PASS; mid triggers EXPLAIN/STACK/BECAUSE/GLOSS/MULTI; low triggers COMPARE/MULTI (outbound copy p2-adversarial capture).
...
- Mid rewrite PASS and low rewrite PASS after one compression each (outbound copy p2-rewrite capture `re_adversarial`).
...
### FM-3 — Closed fail vocab misses documented outbound copy REVISE modes
...
| Operator REVISE history (outbound copy reconcile) | Close/ask anatomy changes | Correctly out of tunnel scope — but shows most REVISE cycles were **not** P2-vocab failures |

**Evidence:** ASP-ASK explicitly lists stand-in + research-meta bans for the second line; FR problem statement cites "wandering paragraph" from outbound copy cycles, then freezes a copy-adjacent seven-id set that omits those bans. Live mid/low FAILs are only the synonym-rich "nice because / personality / compare" cluster the table already names.
...
**Evidence:**
- FR + outbound copy P2-TUNNEL: remediation loop cap **1** (one rewrite, re-run Gate 1).
...
- Outbound copy waveset acceptance: "P2 adversarial PASS (or FAIL→rewrite→PASS)" — assumes the happy transition; no FAIL residual path.
...
**Evidence:**
- FR default scope: "named paragraph / field (default: body paragraph 2)". Assumes outbound copy storefront email shape (opener / profile / close). Options A/C as skill/playbook would export that default into non-email or multi-profile bodies.
- Order: render → tunnel → lint → preview. Tunnel PASS does not check truthfulness against Places/CASE; lint does not check PN-* modes (outbound copy prose capture passed drafts that still needed P2 tunnel — correct ordering, but tunnel remains non-scripted judgment).
...
**Risk:** Reuse outside outbound copy inherits P2 index and judgment-only enforcement; false confidence relative to prose_lint (which is actually runnable).
...
**Evidence:**
- FR cites outbound copy evidence at foreign pin (matches outbound copy ACCEPT capture).
- Tunnel decision + both P2 captures cite a different foreign pin (artifact certification hash).
...
- A scoped adversarial vocabulary can catch the specific mid/low wandering profiles that prose_lint missed (outbound copy prose capture OK vs p2-adversarial capture FAIL).
- Sibling freeze + one rewrite can produce operator-CLEARED one-liners (outbound copy ACCEPT capture).
...
**REVISE** — ship-as-reusable-gate is premature; problem and manual outbound copy pattern are real.
